...
export default function SpacesPage() {
  ...
  const handleInitiateBooking = async (spaceId) => {
    ...
    setError(null);
    try {
        // FIX: Using the correct 'accessToken' key identified in your project
        ...
        const response = await axios.post(
            `${baseUrl}/api/spaces/generate-token/`, 
            { space_id: spaceId },
            { 
                headers: { 
  ...
  return (
    <AppLayout activePage="spaces">
            ...
            <div className="bg-[var(--bg-surface)] border-2 border-[var(--color-accent)] p-8 w-full max-w-sm relative text-center">
                ...
                <div className="mt-6 pt-4 border-t border-dashed border-[var(--border-color)] flex justify-between text-[10px] font-mono">
                    ...
                    <span className="text-[var(--color-accent)]">
                        {bookingToken.meta ? bookingToken.meta.days_total - bookingToken.meta.days_used : '∞'}
                    </span>
                </div>
            </div>
        </div>
      ...
      <div className="mb-6 space-y-6">
        <div className="flex items-end justify-between border-b border-[var(--border-color)] pb-4">
          <div>
            ...
            <p className="text-[var(--text-muted)] text-xs font-mono mt-1">&gt; LOCATING AVAILABLE NODES...</p>
          </div>
        ...
        <div className="flex space-x-1 p-1 bg-[var(--bg-input)] border border-[var(--border-color)] rounded-sm">
            {['all', 'premium', 'standard'].map(filter => (
                <button
                    ...
                    className={`flex-1 py-1.5 text-[10px] font-mono uppercase transition-all rounded-sm ${activeFilter === filter ? 'bg-[var(--color-accent)] text-white' : 'text-[var(--text-muted)]'}`}
                >
                    {filter}
                </button>
            ))}
      ...
      <div className="space-y-4">
        {loading ? (
             <div className="text-center py-12 font-mono text-xs text-[var(--color-accent)] animate-pulse">&gt; DOWNLOADING TOPOLOGY...</div>
        ) : (
            <>
